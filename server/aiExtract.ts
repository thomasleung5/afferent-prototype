/* Pure handler for /api/ai/extract. Takes a Request, returns a Response.
 * Used by both the Hono server (server/index.ts) and the smoke test
 * (scripts/smoke-ai.ts) — same code path. */
...
export async function handleAiExtract(req: Request): Promise<Response> {
  ...
  if (!body.rows || body.rows.length === 0) {
    return json({ ok: true, status: "no-suggestions", suggestions: [] });
  }
