  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22, alignItems: "flex-start" }}>
      <div style={{ background: "var(--paper)", border: "1px solid var(--rule)", padding: 22 }}>
        <div className="mono" style={{ fontSize: "var(--t-l9)", fontWeight: 700, letterSpacing: "0.12em", color: "var(--ink-3)", textTransform: "uppercase", marginBottom: 12 }}>
          Packet sections
        </div>
        {PACKET_SECTIONS.map((s, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: "var(--fs-ui)" }}>{s}</div>
      ...
      <div style={{ background: "var(--paper)", border: "1px solid var(--rule)", padding: 28, fontFamily: "Georgia, serif" }}>
        ...
        <div style={{ fontSize: "var(--fs-ui)", color: "var(--ink-3)", marginTop: 6, marginBottom: 16 }}>
          {jurisdiction.name} · {jurisdiction.preparedBy.split(" · ")[0]}
        </div>
        <div style={{ fontSize: "var(--fs-ui)", color: "var(--ink-2)", lineHeight: 1.7 }}>
          {buildNarrative(summary, fiscalYear)}
        </div>
        <div style={{ marginTop: 16, display: "flex", gap: 8, fontFamily: "var(--ff-ui)" }}>
