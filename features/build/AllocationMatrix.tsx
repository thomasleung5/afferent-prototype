  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Header
        ...
        totalCap={totalCap} stepCount={model.stepOrder.length}
      />
...
function Header({
  view, setView, totalCap, stepCount,
}: {
  view: View; setView: (v: View) => void; totalCap: number; stepCount: number;
}) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, padding: "12px 16px" }}>
        <div className="display" style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.2 }}>
          Allocation matrix · step-down method
        </div>
        <div style={{
          fontSize: 11.5, color: "var(--ink-3)", marginTop: 5,
          lineHeight: 1.5, maxWidth: 780,
        }}>
          Each pool starts on its home indirect dept (Initial). Indirects are then
          closed in the order set on step 1: when a dept is closed, its current
          balance is pushed to every dept BELOW it in the sequence (remaining
          indirects + all directs) using each pool&apos;s own basis. After
          {" "}{stepCount} steps all {fmt.dollarsK(totalCap)} has settled on the
          direct departments.
        </div>
      </div>
      <div style={{
...
function CenterCellTrace({
  center, deptCode, view, model, onClose,
}: {
  center: string; deptCode: MatrixDeptCode; view: View;
          ...
          {contribs.length === 0 ? (
            ...
            <div style={{ border: "1px solid var(--rule)" }}>
              ...
              <div style={{
                ...
                <span>{view === "initial" ? "Initial" : "Final"}</span>
                <span className="num mono" style={{
                  fontVariantNumeric: "tabular-nums",
